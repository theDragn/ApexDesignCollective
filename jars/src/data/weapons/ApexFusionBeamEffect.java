...
public class ApexFusionBeamEffect implements BeamEffectPlugin
{
    ...
    public void advance(float amount, CombatEngineAPI engine, BeamAPI beam)
    {
        ...
        if (beam.getBrightness() >= 1f && target instanceof ShipAPI && !hitTargets.contains(target))
        {
            ...
            if (!hitShield)
            {
                // final check because of weird bug
                //if (MathUtils.getDistanceSquared(ApexUtils.getNearestPointOnShipBounds(ship, beam.getTo()), beam.getTo()) > 25f)
                //    return;
                //System.out.println(MathUtils.getDistanceSquared(ApexUtils.getNearestPointOnShipBounds(ship, beam.getTo()), beam.getTo()));
