...
class ApexExcessionMissileAI(val missile: MissileAPI, val launchingShip: ShipAPI): GuidedMissileAI, MissileAIPlugin
{
    ...
    companion object
    {
        ...
        const val ENGINE_OFF_ANGLE = 90f
    ...
    override fun advance(amount: Float)
    {
        ...
        {
            if (Misc.random.nextFloat() > 0.66f)
            {
                setTarget(
            ...
            } else
            {
                setTarget(
                    MagicTargeting.pickTarget(
                        ...
                        SEARCH_RANGE,
                        SEARCH_CONE,
                        99, FRIGATES, DESTROYERS, CRUISERS, CAPITALS,
                        SEARCH_FAILSAFE
                    )
                )
            }
        ...
        if (timer >= check)
        {
            ...
            if (SPREAD_AMOUNT > 0)
            {
                ...
                if (SPREAD_SHRINK_FACTOR > 0)
                    offset_mult =
                        ApexUtils.lerp(SPREAD_SHRINK_FACTOR, 1f, min(sqrt(targetDist) / SPREAD_SHRINK_DIST, 1f))
