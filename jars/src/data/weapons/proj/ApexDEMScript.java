...
/**
 * Barely modified from base DEM script to give it target prediction, since this fires a projectile.
 */
public class ApexDEMScript extends BaseEveryFrameCombatPlugin implements MissileAIPlugin {
    ...
    public ApexDEMScript(MissileAPI missile, ShipAPI ship, WeaponAPI weapon) {
    ...
    public void advance(float amount, List<InputEventAPI> events) {
        ...
        if (state == State.WAIT && missile.isArmed() && !missile.isFizzling() && !missile.isFading()) {
            ...
            if (targetingLaserRange > 0 && targetingTime > 0) {
                demDrone.giveCommand(ShipCommand.FIRE, AIUtils.getBestInterceptPoint(missile.getLocation(),1000f, fireTarget.getLocation(), fireTarget.getVelocity()), 0);
            }
...
//				p.shapedExplosionMaxParticleDur /= speedMult;
            ...
            WeaponAPI droneWep = demDrone.getAllWeapons().get(1);
            if (droneWep.getAmmo() == 0)
            {
                state = State.DONE;
